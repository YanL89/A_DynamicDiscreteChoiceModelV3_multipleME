...
getFirst = function(spec){
  ...
  if( 0 != ncol(spec$Time)){
		varNamesTime = unlist(strsplit(colnames(spec$Time), ".", fixed = TRUE))
		varNamesTime = varNamesTime[seq(from = 1, by = 2*spec$nTime, 
				to = length(varNamesTime))]
  	} else {
  	varNamesTime = c()
  	}

  if(mean(colnames(First) %in% c(spec$varNames, varNamesTime)) < 1){
